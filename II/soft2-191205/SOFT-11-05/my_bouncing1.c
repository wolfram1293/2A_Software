...
int main(int argc, char **argv){
  ...
  objects[0] = (Object){ .m = 100.0, .y = -19.9,.x=-10.0,.vy = 0.0,.vx=2.0};
  objects[1] = (Object){ .m = 100.0, .y =  19.0,.x=10.0,.vy = 2.0,.vx=0.0};
  objects[2] = (Object){ .m = 100.0, .y =  -19.0,.x=10.0,.vy = 2.0,.vx=0.0};
...
void my_update_velocities(Object objs[], const size_t numobj, const Condition cond){
    ...
    for(int i=0;i<numobj;i++){
        ...
        for(int j=0;j<numobj;j++){
            if(i!=j){
                ax+=cond.G*objs[j].m*(objs[j].x-objs[i].x)/pow(((objs[j].x-objs[i].x)*(objs[j].x-objs[i].x)+(objs[j].y-objs[i].y)*(objs[j].y-objs[i].y)),1.5);
                ay+=cond.G*objs[j].m*(objs[j].y-objs[i].y)/pow(((objs[j].x-objs[i].x)*(objs[j].x-objs[i].x)+(objs[j].y-objs[i].y)*(objs[j].y-objs[i].y)),1.5);
            }
        }
