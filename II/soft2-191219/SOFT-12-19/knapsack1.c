...
int main (int argc, char**argv)
{
  ...
  assert( W >= 0.0);
  int m;
  Itemset *items = load_itemset(argv[3],&m);
  if(n!=items->number){
      printf("<the number of items> command line input:%d file:%d\n",n,m);
      exit(1);
  }
...
Result solve(const Itemset *list,  double capacity)
{
  ...
  Result *max = search(0,list,capacity,flags, 0.0, 0.0);
  free(flags);
  return *max;
}
...
Result *search(int index, const Itemset *list, double capacity, int *flags, double sum_v, double sum_w)
{
  ...
  r->flags=(int*)malloc(sizeof(int)*max_index);
  if (index == max_index){
    ...
    if (sum_w < capacity){
      ...
      r->v=sum_v;
      //r->flags=flags;
      memcpy(r->flags,flags,sizeof(int)*max_index);
      return r;
    } else{
  ...
  flags[index] = 0;
  Result *r0=(Result*)malloc(sizeof(Result));
  r0 = search(index+1, list, capacity, flags, sum_v, sum_w);
  flags[index] = 1;
  Result *r1=(Result*)malloc(sizeof(Result));
  r1->v= 0.0;
  if(sum_w + list->item[index].weight<=capacity){
    r1 = search(index+1, list, capacity, flags, sum_v + list->item[index].value, sum_w + list->item[index].weight);
  }
  // 使った場合の結果と使わなかった場合の結果を比較して返す
  return (r0->v > r1->v) ? r0 : r1;
  free(r0);
  free(r1);
}
...
Itemset *load_itemset(char *filename,int *n)
{
  ...
  itemset->item=(Item*)malloc(sizeof(Item)* *n);

  itemset->number= *n;
  for (int i = 0 ; i <*n ; i++){
    fread(&itemset->item[i].value, sizeof(double), 1, fp);
  }
  for (int i = 0 ; i <*n ; i++){
    fread(&itemset->item[i].weight, sizeof(double), 1, fp);
  }
  fclose(fp);
  return itemset;
}