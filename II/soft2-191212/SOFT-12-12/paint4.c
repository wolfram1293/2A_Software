...
void redo(History *his, Canvas *c);
void fill(Canvas *c, const int x0, const int y0);
...
void redo(History *his, Canvas *c)
{
  ...
  if (p!= NULL){
    ...
    printf("This is latest\n");
  }
}

  void fill(Canvas *c, const int x0, const int y0)
{
  char base=c->canvas[x0][y0];
  int width=c->width;
  int height=c->height;
  int x,y;
  char pen = c->pen;
